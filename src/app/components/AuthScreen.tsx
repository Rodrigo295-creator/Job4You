...
export function AuthScreen({ onAuthenticated }: Props) {
  ...
  const completeAuth = (provider: AuthProvider, sessionEmail?: string, sessionName?: string) => {
    setLoading(provider);
    setError(null);
    window.setTimeout(() => {
      onAuthenticated({
  ...
  const handleSocial = (provider: 'google' | 'apple') => {
    completeAuth(provider, email || undefined, name || undefined);
  };
    ...
    <div className={`min-h-screen flex flex-col lg:flex-row ${isDark ? 'dark bg-[#020617]' : 'bg-white'}`}>
      ...
      <div className="relative lg:w-[48%] xl:w-[52%] shrink-0 overflow-hidden bg-[#0F172A] text-white">
        ...
        <div className="relative z-10 flex flex-col min-h-[280px] lg:min-h-screen p-6 sm:p-10 lg:p-12">
          ...
          <p className="hidden lg:block text-xs text-slate-500 mt-8">{t('auth.demoNote')}</p>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col justify-center px-6 py-10 sm:px-12 lg:px-16 xl:px-20 bg-[#F8F8F6] dark:bg-[#020617]">
        <div className="w-full max-w-md mx-auto">
          ...
          <form onSubmit={handleEmailSubmit} className="space-y-4">
            ...
            {mode === 'login' && (
              <div className="flex justify-end">
                <button type="button" className="text-xs font-semibold text-[#F97316] hover:underline">
          ...
          <p className="lg:hidden mt-4 text-center text-[10px] text-slate-400">{t('auth.demoNote')}</p>
        </div>
      </div>
    </div>
  );
